...
This exercise has a lot of steps and data processing! That means we won't get through all the whole thing. That's okay though! All the material we've presented will stay available in this repository for quite some time (if not forever). We'll breeze through a lot of the exercise, but you can continue the exercise on your own time if you'd like :)
...
- The high resolution DEM was acquired from the [COV's 2013 DEM](https://opendata.vancouver.ca/explore/dataset/digital-elevation-model/). It was clipped to the Fairview extent.
...
Open up ArcGIS to a new project and add a new map. Drag in the `building-footprints-nad.shp` and `elevation-cov-fairview.tif` files. These should be found in `Data/demo-data.zip`.
...
> [!NOTE]
> For the purpose of the live demonstration, we'll only do the first elimination. Feel free to try the others on your own time!
